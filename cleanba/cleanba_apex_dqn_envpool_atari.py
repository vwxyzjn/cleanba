...
def local_buffer_to_replay_buffer(queue, rb, queue_time, make_rollout_time, calculate_priorities_time, rb_add_time):
    while True:
        ...
        priorities = calculate_priorities(rollout, q_state)
...
def rollout(
    ...
    for step in range(args.total_timesteps):
        ...
        store.append((learner_sps, global_step))
        if step % args.log_interval == 0 and step > 0 and thread_id == 0:
            ...
            writer.add_scalar("stats/env_recv_time", np.mean(env_recv_time), global_step)
            writer.add_scalar("stats/get_action_h2d_time", np.mean(get_action_h2d_time), global_step)
            writer.add_scalar("stats/get_action_time", np.mean(get_action_time), global_step)
            writer.add_scalar("stats/get_action_d2h_time", np.mean(get_action_d2h_time), global_step)
            writer.add_scalar("stats/storage_time", np.mean(storage_time), global_step)
            writer.add_scalar("stats/local_buffer_get_time", np.mean(local_buffer_get_time), global_step)
            writer.add_scalar("stats/make_rollout_time", np.mean(make_rollout_time), global_step)
            writer.add_scalar("stats/calculate_priorities_time", np.mean(calculate_priorities_time), global_step)
            writer.add_scalar("stats/replay_buffer_add_time", np.mean(replay_buffer_add_time), global_step)
...
if __name__ == "__main__":
    ...
    learner_sps_store = deque(maxlen=10)
    ...
    for device_id, device  in enumerate(actor_devices):
        ...
        for thread_id in range(args.num_actor_threads):
            ...
            threading.Thread(
                target=rollout,
                args=(args, rb, params_queue, to_use_writer, device_id * args.num_actor_threads + thread_id, device, learner_sps_store)
                ).start()
    ...
    while True:
        ...
        if train_step % 100 == 0:
            ...
            print(f"TSPS: {int(train_step / (time.time() - start_time))} q_value {jax.device_get(q_value).mean():.3f} step {train_step} update time {np.mean(update_time):.3f} batch_queue size {batch_queue.qsize()}")
