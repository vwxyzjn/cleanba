...
def parse_args():
    ...
    parser.add_argument("--learner-device-ids", type=int, nargs="+", default=[0, 1], # type is actually List[int]
...
def prepare_data(
    ...
    env_ids = jnp.hstack(env_ids)
    rewards = jnp.hstack(rewards)

    (agent_state, loss, pg_loss, v_loss, entropy_loss, key) = single_device_update(
        agent_state,
        obs,
        actions,
        logitss,
...
if __name__ == "__main__":
    ...
    prepare_data = jax.pmap(
        prepare_data,
        axis_name="local_devices",
        devices=global_learner_decices,
        axis_size=len(global_learner_decices),
        in_axes=(0, 0, 0, 0, 0, 0, 0, 0, None, None),
        out_axes=(0, 0, 0, 0, 0, None),
        static_broadcasted_argnums=(8),
    )

    obs = [[] for _ in range(len(learner_devices))]
    actions = [[] for _ in range(len(learner_devices))]
    logitss = [[] for _ in range(len(learner_devices))]
    firststeps = [[] for _ in range(len(learner_devices))]
    dones = [[] for _ in range(len(learner_devices))]
    env_ids = [[] for _ in range(len(learner_devices))]
    rewards = [[] for _ in range(len(learner_devices))]
    while True:
        learner_policy_version += 1
        rollout_queue_get_time_start = time.time()
        for device_thread_id in range(args.num_actor_threads * len(args.actor_device_ids)):
            (
                global_step,
                actor_policy_version,
                update,
            ...
            obs[device_thread_id % len(learner_devices)].append(t_obs)
            actions[device_thread_id % len(learner_devices)].append(t_actions)
            logitss[device_thread_id % len(learner_devices)].append(t_logitss)
            firststeps[device_thread_id % len(learner_devices)].append(t_firststeps)
            dones[device_thread_id % len(learner_devices)].append(t_dones)
            env_ids[device_thread_id % len(learner_devices)].append(t_env_ids)
            rewards[device_thread_id % len(learner_devices)].append(t_rewards)
        ...
        agent_state, loss, pg_loss, v_loss, entropy_loss, key = prepare_data(
            agent_state,
            jnp.asarray(obs),
            jnp.asarray(dones),
            jnp.asarray(actions),
            jnp.asarray(logitss),
            jnp.asarray(firststeps),
            jnp.asarray(env_ids),
            jnp.asarray(rewards),
            envs.single_action_space.n,
            key,
