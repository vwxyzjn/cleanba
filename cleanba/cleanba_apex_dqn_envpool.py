...
def make_env(env_id, num_envs, seed):
    def thunk():
        envs = envpool.make(
            ...
            repeat_action_probability=0.25,  # Hessel et al 2022, Muesli paper, Table 4 # NOTE: are we sure this is equivlent to repeat action = 4
            noop_max=1,  # Hessel et al 2022, Muesli paper, Table 4 # NOTE: apex-dqn figure 2 indicates random no-op starts, this needs to be changed to default
...
class QNetwork(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = scale_gradient(x, scale) # # Wang et al. 2016 (Dueling DQN)
...
if __name__ == "__main__":
    ...
    def update(q_state, observations, actions, next_observations, rewards, dones):
        ...
        def mse_loss(params):
            ...
            return rlax.l2_loss(td_error).mean(), q_pred

        (loss_value, q_pred), grads = jax.value_and_grad(mse_loss, has_aux=True)(q_state.params)
        q_state = q_state.apply_gradients(grads=grads)
        grad_norm = jax.tree_map(lambda x: jnp.linalg.norm(x), grads)
        return loss_value, q_pred, grad_norm, q_state
    ...
    while True:
        ...
        loss, old_val, grad_norm, q_state = update(
            q_state,
            data.obs,
            data.actions,
            data.next_obs,
        ...
        # data generation:train ratio
        learner_sps_deque.append(train_step / (time.time() - start_time))
        if len(actor_sps_deque) != 0 and len(learner_sps_deque) != 0:
        ...
        if train_step % 100 == 0:
            ...
            for layer_name, norm in grad_norm['params'].items():
                writer.add_scalar(f"charts/grad/{layer_name}_kernel", norm['kernel'], train_step)
                writer.add_scalar(f"charts/grad/{layer_name}_bias", norm['bias'], train_step)
