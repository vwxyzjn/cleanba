...
if __name__ == "__main__":
    ...
    # env setup
    envs = make_env(args.env_id, args.seed, args.num_envs, async_batch_size=args.num_envs)()
    ...
    for update in range(1, args.num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            returned_episode_returns = np.where(info["terminated"] + truncated, episode_returns, returned_episode_returns)
            episode_returns *= (1 - info["terminated"]) * (1 - truncated)
            episode_lengths += 1
            returned_episode_lengths = np.where(info["terminated"] + truncated, episode_lengths, returned_episode_lengths)
            episode_lengths *= (1 - info["terminated"]) * (1 - truncated)
